...
--fore="#E3E3E3"; sed -i '/welcome-backend.sh &/d' ~/.config/openbox/autostart; if ! grep hypervisor /proc/cpuinfo ; then sed -i '/vmrestore.*/d' ~/.config/openbox/autostart; fi; exit
else
...
--fore="#E3E3E3"; sed -i '/welcome-backend.sh &/d' ~/.config/openbox/autostart; exit
fi
